...
if __name__ == "__main__":
    ...
    # ارجع مجلدين للخلف: من scripts/processing → scripts → fog-detection
    ...
    for file in os.listdir(input_dir):
        if file.endswith(".npz"):
            ...
            # ==== Save NPZ ====
            save_npz = output_dir /"p_daphnet_detection_2sec.npz"
            np.savez(save_npz, X=X_windows, y=y_windows)

            # ==== Save CSV ====
            X_flat = X_windows.reshape(X_windows.shape[0], -1)  # Flatten windows
            df = pd.DataFrame(X_flat)
            df["label"] = y_windows
            save_csv = output_dir  /"p_daphnet_detection_2sec.csv"
